...
bool cmpQu(Q a,Q b){
	if(v[a.l]^v[b.l])return a.l<b.l;
	if(v[a.r]^v[b.r])
	  if(v[a.l]&1)return a.r<b.r;
	    else return a.r>b.r;
	return a.id<b.id;
}
...
int main(){
	...
	for(int i=1;i<=n;i++)v[i]=(i-1)/gh+1;
	...
	for(int i=1;i<=Qu_tot;i++){
		...
		while(l<_l)del(l++);
		while(r>_r)del(r--);
		while(_t>Ch[t+1].id)Time(++t,i);
		while(_t<Ch[t].id)Time(t--,i);
		while(cnt_cnt[Ans])Ans++;
		ans[Qu[i].id]=Ans;
	}
	for(int i=1;i<=q;i++)
	  if(ans[i])printf("%d\n",ans[i]);
