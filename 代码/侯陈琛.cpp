		...
		if(select==1){
			...
			do
			{
				...
				if(a>number){
					e++;
					if(e<2){
						printf("侯陈琛唉，这也太大了吧，一个国庆过nt了？再让你猜一次："); 
					}else if(e<4){
						printf("唉，我的天，还是大了，你真nt了？再来："); 
					}else{
						printf("大了大了，加油，继续："); 
					}
					//scanf("%d",&a);
					...
					f++;
					if(f<2){
						printf("侯陈琛唉，你是怎么想的，猜这么小干什么，nt了？？？再让你猜一次："); 
					}else if(f<4){
			...
			if(count<=7){
				...
				printf("哟呵，儿子还不错嘛，才%d次就猜到了，和你爸爸心有灵犀哦。\n",count);
			}else{
				int n=1;
				while(n--){
					p++;
				}
				printf("你个呆皮用了%d次才猜中？？？？你个傻狗，多学学数学吧，没救了......\n",count);
			}
		}else if(select==0){
			i++;
			system("color fC");
